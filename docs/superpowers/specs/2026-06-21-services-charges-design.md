...
`playCount(i)` — **один и тот же таймлайн** для desktop (триггер на snap-активацию count'а)
и mobile (триггер on-enter). DRY. **Звука нет** — слэм чисто визуальный.

**Финал (`SENTENCE`):** после count 05 — экран вердикта: `GUILTY ON ALL FIVE COUNTS` (slam,
как штампы) + строка приговора. Без кнопок.

## 4. Поведение на мобиле (M2, через `gsap.matchMedia()`)

- **Без пиннинга.** Counts — вертикальная стопка экранов; `playCount(i)` триггерится
  **on-enter** при входе count'а во вьюпорт (механика как у `DossierMotion` в About — проверена).
- Правый рейл сворачивается в **тонкий номер `0i / 05`** на карточке.
- Финал `SENTENCE` — последняя карточка стопки.
- Нативный скролл, ноль скролл-джекинга.
