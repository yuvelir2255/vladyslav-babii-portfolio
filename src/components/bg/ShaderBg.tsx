...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    // тёплый бетон под токены: #100f0d (--color-bg) → #2a2620 (--color-surface).
    // раньше base был нейтрально-серым → ПК-фон холоднее мобильного (там виден
    // тёплый CSS-градиент, шейдер не грузится). выровняли температуру.
    vec3 base = mix(vec3(0.063, 0.059, 0.051), vec3(0.165, 0.149, 0.125), c);

    // тёплый прожектор у курсора
    float d = distance(uv, uMouse);
    base += vec3(1.0, 0.45, 0.18) * smoothstep(0.45, 0.0, d) * 0.12;
