...
**Статус (2026-06-20): hero собран, идёт фаза 4.** Направление утверждено — концепт **«Case File» (тюрьма/дело: тёмный бетон + сталь + prison-orange, метафора «неудержимый билдер, сбегающий из клетки демок»)**. Сборка идёт в ветке **`feat/hero-case-file`** (в `main` НЕ смержено).

- **Сделано:** фундамент (токены/шрифты), статичный hero (2 экрана), весь моушн (Lenis / SplitText-имя / line-reveal манифеста / курсор-прожектор / OGL-шейдер бетона), плавающая навигация **FileNav**, рукописная booking-табличка на мугшоте.
- **Осталось — фаза 4:** прелоадер «lockdown», тумблер звука, финальный адаптив/a11y → потом `impeccable`, merge в `main` + деплой (по «ок»).
...
- **Структура:** `src/components/{hero,bg,motion,cursor,chrome}`, контент в `src/content/hero.ts`, GSAP-реестр `src/lib/gsap.ts`. Страница: `src/app/page.tsx` (ConcreteBg + Cursor + FileNav + Hero).
