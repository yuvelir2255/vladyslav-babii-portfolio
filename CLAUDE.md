...
- 🔄 **Фаза 3** — секции героя и манифеста:
  ...
  - ⏭️ **Task 3.4** About — СЛЕДУЮЩИЙ ШАГ: блок «кто я» из спеки §5.3 (EN/UK), reveal по абзацам (`sections/About.tsx`), добавить в `page.tsx` + namespace `About` в en/uk.json. Затем коммит и закрыть Фазу 3.
- ⏭️ Далее после Фазы 3: **4** карточки What I do → **5** подпись «Ready when it matters.» + кейс Dream Gold + контент-модель (TS+MDX) → **6** контакт-форма→Telegram → **7** i18n (тест паритета + полный перенос) → **8** SEO/OG + графика (Higgsfield) → **9** полировка (impeccable/a11y/перф) → **10** деплой.
