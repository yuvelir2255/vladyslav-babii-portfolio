...
- ⏭️ **СЛЕДУЮЩИЙ ШАГ — Фаза 5**: подпись «Ready when it matters.» (`sections/Signature.tsx`) + контент-модель проектов (TS+MDX: `content/projects/types.ts` + dream-gold-app `live`, dream-gold-site `coming-soon`, тест валидности) + кейс Dream Gold (`sections/WorkDreamGold.tsx`, фото с телефона предоставит владелец; при нехватке — Higgsfield) + слот «More projects soon». Детали — план §Фаза 5 (Task 5.1–5.3).
- ⏭️ Далее: **6** контакт-форма→Telegram → **7** i18n (тест паритета + полный перенос) → **8** SEO/OG + графика (Higgsfield) → **9** полировка (impeccable/a11y/перф) → **10** деплой.
