...
- 🔄 **Фаза 2** — WebGL-поле (`src/components/field/Field.tsx`): ✅ дюны с глубиной/горизонтом, ✅ звёздное небо с мерцанием (Transform-сцена, отдельная программа), ✅ реакция на курсор — параллакс камеры за мышью (сглажен), ✅ провод скролл-камеры (uScroll по прогрессу скролла; активен, когда появятся секции). Проверено в браузере (reduced-motion): орб-курсор и поле рисуются. ⏭️ Перенесено в **Фазу 3**: слова BUILD/CRAFT «вылепленные» из частиц (фолбэк — DOM-текст) — делать вместе с манифестом.
