...
**Статус (2026-06-21): ПРОД (`main`) = пока ТОЛЬКО hero (Case File).** Концепт утверждён — **«Case File» (тюрьма/дело: тёмный бетон + сталь + prison-orange, метафора «неудержимый билдер, сбегающий из клетки демок»)**.

> 🔜 **Готово на ветке `feat/about-the-inmate` (PR #1, preview-деплой, ЕЩЁ НЕ в проде — ждёт решения владельца о merge):**
> секция **About «The Inmate»** (рассекречиваемое досье) + переделан эффект **манифеста** (слово-за-словом подсветка по скроллу, пин по центру) + **единый фон** на весь сайт (один `ConcreteBg`, убраны per-section grain/bars) + сняты разделители-полоски между секциями + **курсор одинаковый** везде (убран scan-вариант) + воздух в прелоадере. ⚠️ При merge в прод уедет не только About, но и правки hero (манифест/фон/курсор) — сверка scope обязательна (`deploy-scope-check-before-prod`).
...
- **Сделано (на проде, `main`):** фундамент (токены/шрифты), hero (2 экрана), моушн (Lenis / SplitText-имя / ScrambleText-манифест / курсор-прожектор / OGL-шейдер), FileNav, booking-табличка, прелоадер «lockdown», тумблер звука (`/audio/breakbeat.mp3`), адаптив + a11y. `impeccable` пройден.
- **Сделано (на ветке `feat/about-the-inmate`, ждёт merge):** About — `src/components/about/*` + `src/content/about.ts`; моушн `DossierMotion` (on-enter reveal: фото+текст одновременно сверху-вниз, declassify грифов по клику/тапу, count-up возраста, DrawSVG-отпечаток, штамп) — **без пиннинга** (по фидбэку владельца). 2 новых фото «билдер в камере»: `public/media/booking/inmate-build.webp` (большое, цвет) + `inmate-detail.webp` (малое, ч/б), Nano Banana Pro. Манифест — `ManifestMotion`: pin+scrub word-highlight (matchMedia: десктоп `+320%` scrub 1, мобайл `+150%` scrub 0.6). Фон — единый `ConcreteBg` (per-section grain/bars сняты, border-t-разделители убраны). Курсор — без scan-варианта (одинаковый везде).
- **СЛЕДУЮЩАЯ ЗАДАЧА (по порядку, после merge About):** (1) секция **services/«Charges»**, затем **work/«Evidence»** — оживить навигацию FileNav (каждая с премиум-моушном, планка hero/about); (2) **вернуть контактную форму → Telegram** (портировать из archive); (3) **вернуть SEO** (sitemap/robots/OG); (4) лицо: Soul ID при 3–5 фото. Конвейер на секцию: brainstorming→план → дизайн-скиллы → `impeccable`. **Сайт EN-only — i18n отменён.**
...
- **Структура:** `src/components/{hero,about,bg,motion,cursor,chrome}`, контент в `src/content/{hero,about}.ts`, GSAP-реестр `src/lib/gsap.ts`. Страница: `src/app/page.tsx` = Preloader + ConcreteBg + Cursor + FileNav + `<main>`(Hero + About) + AudioToggle. На ветке about `<main>` переехал из `Hero` в `page.tsx` и оборачивает Hero+About (Hero теперь отдаёт фрагмент).
...
- **FileNav:** плавающий чип сверху по центру; секции с ироничным неймингом (THE YARD/THE INMATE/CHARGES/EVIDENCE/VISITING HOURS); анимирована (gsap). Якорь `#about` живой (на ветке); `#services/#work/#contact` оживут по мере добавления секций.
- **Фон (единый, на ветке about):** один `ConcreteBg` (fixed: радиальный градиент + OGL-шейдер + grain + bars) на весь сайт. Per-section `grain`/`bars` и border-t-разделители сняты — секции прозрачны над общим фоном. Курсор один и тот же везде.
...
- **`main` = живой прод (hero-only).** Прод-URL: **https://portfolio-vladyslav-s-projects9.vercel.app** (+ алиас `…-git-main-…`). GitHub: https://github.com/yuvelir2255/vladyslav-babii-portfolio (PUBLIC). `feat/hero-case-file` смержена в `main`. **Открыт PR #1 `feat/about-the-inmate` → `main`** (About + правки hero: манифест/фон/курсор) — preview: `https://portfolio-git-feat-about-the-inmate-vladyslav-s-projects9.vercel.app`, ждёт решения владельца о merge.
