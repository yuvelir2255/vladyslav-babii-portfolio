...
- **СЛЕДУЮЩАЯ ЗАДАЧА (явно от владельца): достраивать сайт ПОСТЕПЕННО, секция за секцией — каждую с лучшими фишками / технологиями / анимациями** (планка hero: GSAP ScrollTrigger/SplitText/Flip, Lenis, OGL-шейдеры, кастомный курсор, продуманный premium-моушн). Конвейер на каждую секцию: brainstorming→план → `frontend-design`/`design-taste-frontend`/`ui-ux-pro-max` → `impeccable`. Готово (hero + манифест) проверено и на проде. Ориентир порядка: (1) секции about/«The Inmate», services/«Charges», work/«Evidence» — оживить мёртвую навигацию; (2) **вернуть контактную форму → Telegram** (лидоген, портировать из archive); (3) **вернуть SEO** (sitemap/robots/OG); (4) лицо: Soul ID при 3–5 фото (сейчас 1). **Сайт EN-only — i18n/украинская локаль отменены владельцем (2026-06-20).**
...
- **`main` = живой прод (hero-only).** Прод-URL: **https://portfolio-vladyslav-s-projects9.vercel.app** (+ алиас `…-git-main-…`). GitHub: https://github.com/yuvelir2255/vladyslav-babii-portfolio (PUBLIC). `feat/hero-case-file` уже смержена в `main`.
