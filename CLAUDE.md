...
| About (`#about`) | `components/about` · `content/about.ts` | `DossierMotion` — **scroll-scrub** (ревил привязан к скроллу, per-element, без пина) | досье; declassify грифов; count-up возраста; DrawSVG-отпечаток; 2 фото «билдер в камере» (`inmate-build.webp` цвет + `inmate-detail.webp` ч/б) |
...
| Contact (`#contact`) | `components/contact` · `content/contact.ts` | `ReleaseMotion` — стадия 1 **scroll-scrub** + стадия 2 по событию `vb:released` | «Release» (Cell Door + свет): прутья садятся → на отправке разъезжаются (`xPercent ±110`) + дневной свет (feathered-шафт + blur); форма 3 поля → Telegram; типографская directory |
