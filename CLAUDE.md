...
**Статус (2026-06-21): ПРОД (`main`) = hero + About «The Inmate» + Services «Charges» + Work «Evidence» + Contact «Visiting Hours» (Case File) — НАРРАТИВ САЙТА ЗАВЕРШЁН.** Концепт утверждён — **«Case File» (тюрьма/дело: тёмный бетон + сталь + prison-orange, метафора «неудержимый билдер, сбегающий из клетки демок»)**. Секция Contact смержена в `main` локально (`--no-ff`, ветка `feat/contact-visiting-hours` удалена); **push в `main` = прод-деплой Vercel** (env `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` уже добавлены на Vercel, Production+Preview).
