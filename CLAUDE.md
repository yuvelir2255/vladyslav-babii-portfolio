...
- **СЛЕДУЮЩАЯ ЗАДАЧА (по порядку):** (1) секция **VISITING HOURS (`#contact`)** — финал сайта: контакт + **форма-заявка → Telegram** (портировать из archive `sections/Contact.tsx` + `api/contact`); владелец хочет её **оригинальной, с плавными анимациями/переходами** — начать с `brainstorming`; (2) **SEO** — остались `sitemap`/`robots` (OG/og-image уже сделаны); (3) **P1-связки** (теперь #work есть): hero-CTA «Open the case file ↗» → `#work`, «Make contact» → `#contact` (после п.1), FileNav `#contact`; (4) лицо: Soul ID при 3–5 фото. Конвейер на секцию: brainstorming→план → дизайн-скиллы → `impeccable`. **Новые секции — на feature-ветке от свежего `main`.** **Сайт EN-only — i18n отменён.**
