...
- ⏭️ **СЛЕДУЮЩИЙ ШАГ — Фаза 7 (i18n)**: тест паритета ключей en/uk (`tests/i18n-parity.test.ts`) + ревизия, что весь копирайт в сообщениях. Переключатель уже есть. (Большая часть копирайта уже двуязычна по ходу.)
- ⏭️ Далее: **8** SEO/OG + графика (Higgsfield) → **9** полировка (impeccable/a11y/перф) → **10** деплой (новый GitHub-репо + Vercel; домен в самом конце).
