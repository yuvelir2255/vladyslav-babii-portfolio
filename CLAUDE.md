...
- ⏭️ **СЛЕДУЮЩИЙ ШАГ — Фаза 8 (SEO/OG + графика)**: локализ. `generateMetadata` (title/description, canonical + hreflang en/uk/x-default, один `<h1>`), OG-картинки `next/og` (кириллица-safe), `sitemap.ts`, `robots.ts`; при нужде графика через Higgsfield.
- ⏭️ Далее: **9** полировка (impeccable/a11y/перф — здесь дизайн-конвейер по просьбе владельца) → **10** деплой (новый GitHub-репо + Vercel; домен в самом конце).
