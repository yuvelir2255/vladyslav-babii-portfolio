...
- ⏭️ **СЛЕДУЮЩИЙ ШАГ — Фаза 10 (деплой)**: новый GitHub-репо (`master`→`main`) + Vercel (env `TELEGRAM_*`, по желанию `NEXT_PUBLIC_SITE_URL`), проверка прод-URL (форма в Telegram, обе локали, OG), затем (по одобрению) свой домен и переключение со старого `about-me-website`. Опц. до деплоя: Lighthouse-замер (9.2), in-page «reduce motion» toggle (если передумает).
...
- ⏳ **Осталось:** (a) тест формы → Telegram на проде (нужен 1 тестовый сабмит — отправляет в Telegram владельца, спросить ок); (b) подтвердить, подключён ли Git для push-деплоя (Settings→Git; проект создан CLI-аплоадом, не Git-linked); (c) почищенный дубль `.vercel` в `.gitignore` — не закоммичен.
- **Свой домен** — в самом конце по отдельному «ок». Старый `about-me-website` (статика на Vercel с автодеплоем) живёт до переключения; не трогать.
- Локальный твик: CLI дописал дубликат `.vercel` в `.gitignore` — почищено (ещё не закоммичено).
